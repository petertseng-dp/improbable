...
class VennDiagram
  ...
  #
  # !!! Again, I'd like to use UInt32 for this, but Array#size is an Int32
  @knowns : Hash(Set(Int32), Int32)

  def initialize(@vars : Array(String))
    @var_indices = @vars.each_with_index.to_h
    all_regions = (0...size).to_set
  ...
  def learn(fact : String)
    ...
    learn_and_infer(regions_of(vars), probability.to_i, "GIVEN")
  end

  def query(question : String) : Int32?
    puts "I now know #{@knowns.size} region-sets"
    @knowns[regions_of(question)]?
  end

  def regions_of(vars : String) : Set(Int32)
    vars = vars.split(" & ")
    known_values = vars.map { |v|
      v[0] == '!' ? {false, @var_indices[v[1..-1]]} : {true, @var_indices[v]}
    }
    known_portion = known_values.map { |on, pos| (on ? 1 : 0) << pos }.reduce(0) { |a, b| a | b }
    unknown_bits = (0...@vars.size).to_set - known_values.map { |_, pos| pos }.to_set
    regions = unknown_bits.reduce([known_portion]) { |regs, bit| regs.flat_map { |r| [r, r | (1 << bit)] } }.to_set
  end

  private def learn_and_infer(regions : Set(Int32), probability : Int32, comment : String)
    if @knowns[regions]?
